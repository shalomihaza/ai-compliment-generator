...
COMPLIMATIC generates three over-the-top compliments in three distinct comedic _registers_ from a job title / person details, each escalatable through 5 drama levels. The product thesis is **honest strictness**: every compliment must comply with 8 brand "Compliment Style Guidelines," and the UI only shows a checkmark for rules the code can actually verify.
...
- Tier 3 prompt-enforced only: rules 1/7/8 and the _quality_ of 2/3/4 — deliberately **not** claimed as validated.
