 ...
 *
 * The validator disclosure ("checked automatically") is deliberate — telling
 * the model its word count and banned words are machine-checked measurably
 * improves compliance.
 */
...
export const BRAND_RULES = [
  ...
  "Every compliment must include one made-up statistic",
...
function brandRulesBlock(): string {
  const rules = BRAND_RULES.map((rule, i) => `${i + 1}. ${rule}`).join("\n");
  return `## Brand rules (hard constraints — every compliment, every time)
${rules}
Rules 5 (max 40 words) and 6 (no "literally") are checked automatically by a
validator after you respond. Count your words.`;
}

const EVIDENCE_CONTRACT = `## Evidence
For each compliment, fill rules_satisfied with VERBATIM quotes from your own
compliment text: the phrase referencing the job (job_reference), the absurd
metaphor (absurd_metaphor), and the made-up statistic (fake_statistic). An
automated check verifies each quote appears word-for-word in the text — do not
